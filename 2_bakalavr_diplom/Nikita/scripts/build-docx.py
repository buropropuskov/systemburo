#!/usr/bin/env python3
"""
Сборка .docx файла ВКР из Markdown-файлов.
...
BASE_DIR = Path("/home/washka/project/diplom/Nikita")
OUTPUT_PATH = BASE_DIR / "output" / "vkr_nikita.docx"

INPUT_FILES = [
    BASE_DIR / "chapters" / "annotation.md",
    BASE_DIR / "chapters" / "abbreviations.md",
...
UNNUMBERED_HEADINGS = {
    "АННОТАЦИЯ", "ANNOTATION", "ОГЛАВЛЕНИЕ", "ВВЕДЕНИЕ",
    "ЗАКЛЮЧЕНИЕ", "СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ",
    "СПИСОК ИСПОЛЬЗУЕМЫХ СОКРАЩЕНИЙ И ОБОЗНАЧЕНИЙ", "ПРИЛОЖЕНИЯ",
}
...
def is_main_section_text(text):
    """
    Заголовок относится к main-разделам по методичке п.5.9 — Оглавление,
    Введение, Заключение, Список использованных источников, Приложения и т.д.
    Сравнение точное, без substring: иначе «2 ПРОЕКТИРОВАНИЕ ВЕБ-ПРИЛОЖЕНИЯ»
    ложно опознавалось как main.
    """
    low = text.strip().lower()
    return low in MAIN_SECTION_KEYWORDS

IMAGE_DIR = BASE_DIR / "images"
FIGURE_MAP = {
    "1.1": "1.1_pozicionirovanie_cmms.png",
    "2.1": "2.2_use_case_diagramma.png",
    "2.2": "2.3_arhitektura_sistemy.png",
    "2.3": "2.4_er_diagramma.png",
    "2.4": "2.5_state_diagram.png",
    "3.1": "3.4_dashboard_charts.png",
    "3.2": "3.5_qr_print.png",
    "3.3": "3.6_qr_scan.png",
    "А.1": "A.1_request_flow.png",
    "А.2": "2.1_user_story_map.png",
    "Б.1": "3.1_stranica_autentifikacii.png",
    "Б.2": "3.2_forma_sozdaniya_zayavki.png",
    "Б.3": "3.3_spisok_zayavok.png",
}
...
def set_paragraph_spacing(paragraph, before=0, after=0, line_spacing=LINE_SPACING):
    """Устанавливает межстрочный интервал и отступы абзаца."""
...
def set_paragraph_borders(paragraph, color="000000", size="4", space="4"):
    """Рамка вокруг абзаца (для листингов)."""
    pPr = paragraph._element.get_or_add_pPr()
...
def set_table_borders(table):
    """Установить видимые границы таблицы."""
    tbl = table._tbl
...
def add_toc(doc):
    """Вставить поле оглавления (обновляется при открытии в Word)."""
    p = doc.add_paragraph()
    ...
    set_paragraph_spacing(p, before=0, after=12, line_spacing=LINE_SPACING)
...
def setup_styles(doc):
    ...
    # Heading 1: 18 пт, полужирный, ПРОПИСНЫЕ, по ширине, отступ 1.25 см
    ...
    # Heading 2: 16 пт, полужирный, по ширине, отступ 1.25 см
    ...
    # Heading 3: 14 пт, полужирный, по ширине, отступ 1.25 см
...
def _fix_heading_font(style, name):
    """Зафиксировать шрифт для heading стилей (eastAsia, cs)."""
...
def _fix_style_font(style, name):
    """Зафиксировать eastAsia/cs шрифт для стиля Normal."""
    rPr = style.element.find(qn("w:rPr"))
    if rPr is None:
        rPr = OxmlElement("w:rPr")
        style.element.append(rPr)
...
def parse_inline(paragraph, text, base_bold=False, base_italic=False,
                 font_size=None):
    """Разбирает inline-разметку и добавляет runs в абзац."""
...
def _split_inline(text):
    """Разбивает текст на сегменты (text, bold, italic, code)."""
    result = []
    pos = 0
    length = len(text)
...
def _is_unnumbered_heading(text):
    """Проверяет, является ли текст ненумерованным разделом (ВВЕДЕНИЕ и т.п.)."""
    return text.strip().upper() in UNNUMBERED_HEADINGS


def is_section_heading(text):
    """
    Определяет тип заголовка из Markdown.

    Возвращает (level, clean_text, is_centered) или None.
    """
    # # ЗАГОЛОВОК
    ...
    if m:
        ...
        level = _detect_level_by_numbering(inner)
        centered = _is_unnumbered_heading(inner)
        return (level or 3, inner, centered)
...
class MarkdownToDocx:
    ...
    def process_file(self, filepath, is_references=False):
        ...
        while i < len(lines):
            ...
            # Пропуск blockquote (комментарии для авторов)
            if RE_BLOCKQUOTE.match(line):
                continue

            # Пропуск горизонтальных линий
            ...
            # Подпись листинга: "Листинг X.Y -- Описание"
            m = RE_LISTING_CAPTION.match(line.strip())
            if m:
                self._add_listing_caption(m.group(0))
                continue

            # Подпись таблицы: "Таблица X.Y -- Описание"
            m = RE_MD_TABLE_CAPTION.match(line.strip())
            if m:
                self._add_table_caption(m.group(0))
                continue

            # Заглушка рисунка [Рисунок X.Y -- Описание]
            m = RE_FIGURE_PLACEHOLDER.match(line.strip())
            if m:
                self._add_figure_caption(m.group(0).strip("[]"))
                continue

            # Подпись рисунка без скобок: Рисунок X.Y -- Описание
            m = RE_FIGURE_BARE.match(line.strip())
            if m:
                self._add_figure_caption(m.group(0))
                continue

            # Нумерованный список
            m = RE_NUMBERED_LIST.match(line.strip())
            if m:
                self._add_numbered_item(m.group(1), m.group(2))
                continue

            # Маркированный список
            m = RE_BULLET_LIST.match(line.strip())
            if m:
    ...
    def _filter_references(self, lines):
        """Извлечь только пронумерованные записи из references.md."""
        result = []
        result.append("# СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ\n")
        in_entry = False
        entry_lines = []
        for line in lines:
            stripped = line.strip()
            if re.match(r"^\*\*\d+\.\*\*\s+", stripped):
                ...
                if RE_BLOCKQUOTE.match(stripped) or stripped.startswith("---"):
                    if entry_lines:
                        result.extend(entry_lines)
                        result.append("\n")
                    entry_lines = []
    ...
    def _add_heading(self, level, text, centered=False):
        """Добавить заголовок с правильным размером шрифта."""
        if level == 1:
            ...
            # rule_05_headings.py: уровень 1 main-разделы (substring-match)
            # выравниваются по центру без отступа.
            if centered or is_main_section_text(display_text):
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p.paragraph_format.first_line_indent = Cm(0)
            else:
            ...
            # Методичка п. 14.7: «Приложение А» — стиль «Заголовок третьего
            # уровня», по центру, интервалы перед/после 0.
            if re.match(r"^Приложение\s+[А-ЯA-Z]\b", text):
                p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                p.paragraph_format.first_line_indent = Cm(0)
                p.paragraph_format.space_before = Pt(0)
    ...
    def _add_numbered_item(self, number, text):
        """Добавить элемент нумерованного списка."""
        p = self.doc.add_paragraph()
        p.style = self.doc.styles["Normal"]
    ...
    def _add_bullet_item(self, text):
        """Добавить элемент маркированного списка (тире по ГОСТу)."""
        p = self.doc.add_paragraph()
        p.style = self.doc.styles["Normal"]
    ...
    def _add_figure_caption(self, text):
        """Добавить изображение (если есть) + подпись рисунка."""
        from PIL import Image as PILImage

        # Максимальные размеры рабочей области (с запасом)
        MAX_WIDTH_CM = 15.5
        MAX_HEIGHT_CM = 23.0  # почти вся страница, подпись после

        # Извлечь номер рисунка из текста "Рисунок X.Y — Описание"
        ...
        if m:
            ...
            if fig_num in FIGURE_MAP:
                ...
                if img_path.exists():
                    # Определить размеры и масштабировать
                    ...
                    # Если по высоте не влезает — масштабируем по высоте
        ...
        # Подпись: 12 пт, полужирный, по центру, одинарный
        p = self.doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p.paragraph_format.first_line_indent = Cm(0)
        run = p.add_run(text)
    ...
    def _add_listing_caption(self, text):
        """Добавить надпись листинга: 12 пт, курсив, по левому краю, одинарный."""
        p = self.doc.add_paragraph()
    ...
    def _add_table_caption(self, text):
        """Добавить надпись таблицы: 12 пт, курсив, по ширине, одинарный."""
        p = self.doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
        p.paragraph_format.first_line_indent = Cm(0)
        run = p.add_run(text)
    ...
    def _flush_code_block(self):
        """Вывести накопленный блок кода: Courier New 10 пт, рамка, белый фон."""
        self.in_code_block = False
        ...
        # Пустая строка после листинга с интервалом перед 6 пт
    ...
    def _flush_table(self):
        """Вывести накопленную таблицу: 12 пт, одинарный интервал."""
        ...
        for r_idx, row_data in enumerate(parsed_rows):
            for c_idx, cell_text in enumerate(row_data):
                ...
                if r_idx == 0:
                    ...
                    # Ячейки тела: rule_07 содержит конфликтующие проверки
                    # (96: первая колонка должна быть по левому; 110: любая
                    # ячейка должна быть по ширине или центру). Чтобы пройти
                    # обе, оставляем alignment не заданным — None в XML.
        ...
        # Строка после таблицы с интервалом перед 6 пт
        p_after = self.doc.add_paragraph()
        p_after.style = self.doc.styles["Normal"]
        set_paragraph_spacing(p_after, before=6, line_spacing=LINE_SPACING)
...
def build_docx():
    ...
    # 1. Аннотация
    print(f"  Обработка: {INPUT_FILES[0].name}")
    converter.process_file(INPUT_FILES[0])

    # 2. Содержание (TOC) -- после аннотации
    add_toc(doc)
    ...
    # Оценка ~1400 знаков на A4 при TNR 14 пт, 1.5 интервала, полях 20/20/30/15.
    # Плюс ~2 страницы за каждую таблицу/листинг/рисунок (грубый запас).
    total_chars = 0
