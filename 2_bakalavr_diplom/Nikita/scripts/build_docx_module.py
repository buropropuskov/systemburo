...
def is_main_section_text(text):
    """
    Проверка, что заголовок относится к main-разделам по правилам нормоконтроля
    (substring-совпадение, как в rule_05_headings.py системы Астафьева).
    Из-за этого «2 ПРОЕКТИРОВАНИЕ ВЕБ-ПРИЛОЖЕНИЯ» считается main.
    """
    low = text.lower()
    return any(k in low for k in MAIN_SECTION_KEYWORDS)
