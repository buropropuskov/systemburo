...
Что делать: взять ветку PR, пересобрать перечень и дослать коммит туда же.

```bash
git fetch origin && git checkout <ветка-PR-бота>
make notices          # python3 scripts/gen-third-party-notices.py
git commit -am "build(deps): пересобрал перечень компонентов" && git push
```
