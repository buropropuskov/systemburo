...
func printFakePlan(profile fakedata.Profile, plan []fakedata.PlanItem) {
	...
	// число -- потолок. До поста доходят только машины и люди принятых в работу заявок
	// с непросроченным пропуском, и фактически отмеченных всегда меньше. Пока строки
	// стояли в общей таблице и входили в «всего создастся», отчёт о наливке выглядел
	// так, будто часть работы потерялась.
	if marks := fakedata.PlanMarks(plan); len(marks) > 0 {
		fmt.Println()
		fmt.Println(padRight("Что будет отмечено", 30), padLeft("Не более", 10))
