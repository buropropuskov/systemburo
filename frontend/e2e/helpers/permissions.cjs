...
const createRole = (req, t, data) => apiPost(req, t, '/roles', data);
...
const createGroup = (req, t, data) => apiPost(req, t, '/permission-groups', data);
...
const mergeGroups = (req, t, data) => apiPost(req, t, '/permission-groups/merge', data);
...
/**
 * Удаляет все сущности с префиксом e2e_ (best-effort, без throw).
 * Вызывается в afterAll/afterEach чтобы staging не зарастал тестовыми данными.
 */
async function cleanupE2eEntities(request) {
  const token = await loginAsSuperAdmin(request);

  for (const role of await listRoles(request, token).catch(() => [])) {
    if ((role.code || '').startsWith(E2E_PREFIX) || (role.name || '').startsWith(E2E_PREFIX)) {
      await deleteRole(request, token, role.id).catch(() => {});
    }
  }
  for (const group of await listGroups(request, token).catch(() => [])) {
    if ((group.name || '').startsWith(E2E_PREFIX)) {
