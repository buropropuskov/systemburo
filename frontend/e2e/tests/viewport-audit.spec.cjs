...
// 834 - iPad Pro 11 в портрете: между 810 и 1024 лежит полоса, где двухколоночная
// раскладка уже включена, а места ей ещё не хватает. Обход без этой ширины пропустил
// «Обзор и новости», где документ был 875 при вьюпорте 834.
...
// принятую норму.
//
// На планшете норма 24 - это минимум WCAG 2.2, и ниже него контрол получает
// невидимую зону нажатия псевдоэлементом (`tablet.css`), а видимый размер остаётся
// десктопным. Через размер узла такую зону не измерить, поэтому гейт смотрит только
// на нижнюю границу.
//
// 36 и 32 здесь были и оба раза получили от владельца одно и то же: «жирные и
// огромные», «воздуха нет». Шапка блока 35px, кнопка на десктопе 25 и вокруг 10px
// воздуха; поднятая до 32 занимает шапку целиком.
const TOUCH_MIN_PHONE = 36;
const TOUCH_MIN_TABLET = 24;
