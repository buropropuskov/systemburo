...
export default {
  ...
  computed: {
     ...
     */
    /**
     * Режим смены страниц: собранное приложение дожидается ухода прежней страницы
     * (out-in), dev-сервер - нет.
     *
     * На dev-сервере Vite переход out-in не отпускает страницу: уход отрабатывает и
     * прежний компонент исчезает, а следующий не монтируется никогда - после входа
     * остаётся пустой экран, и дальнейшие переходы его не оживляют. В собранном
     * приложении этого нет: вход отрабатывает и на мгновенных ответах, и на
     * придержанных. Разница именно между сборкой и dev-сервером, не в нашем коде:
     * симптом держится и без HMR, и с production-режимом dev-сервера, и на чистом
     * дереве зависимостей. Пока причина не найдена, разработка идёт без режима -
     * страницы на четверть секунды накладываются, но экран живой.
     */
    pageTransitionMode() {
      return import.meta.env.DEV ? undefined : 'out-in';
    },
    consentBlocking() {
 ...
 */
/* Лист вместо окна - контракт ТЕЛЕФОНА (эталон §3), и порог у него 767.98.
 *
 * До этого лист включался до 899.98, и на планшете в портрете окно превращалось в
 * растянутую мобилку: на 768 «Выбор существующих сотрудников» занимал всю ширину
 * экрана, прижимался к нижнему краю и оставлял сверху 561px пустоты - при том, что
 * на 1024 то же окно выглядит нормальным диалогом 900x472 по центру. Планшет
 * получает диалог; геометрию он считает сам, как на десктопе.
 */
...
/* Эти два правила не про геометрию листа и потому живут до 899.98, а не до 767.98:
   тач-таргеты нужны и планшету, а зум по фокусу поля iPad делает так же, как iPhone. */
