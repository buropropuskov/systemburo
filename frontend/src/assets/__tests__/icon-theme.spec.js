...
describe('icon-theme.css', () => {
  ...
  it('уцелевшая переменная объявлена и в светлой, и в тёмных темах', () => {
    // Из трёх ролей осталась одна - на неё ссылается FactTable. Объявление обязано
    // быть в обеих темах, иначе фильтр в одной из них станет пустым. Замок держит
    // только живую переменную: мёртвые ушли вместе с растром, и требовать их
    // объявления значило бы охранять код, который никто не читает.
    expect(CSS.match(/--icon-ink-filter:/g) ?? [], 'объявлена не во всех темах').toHaveLength(2);
    ['--icon-mono-filter', '--icon-accent-filter'].forEach((v) => {
      expect(CSS.includes(v), `${v} больше никем не читается - место ей в истории`).toBe(false);
    });
  });
});
