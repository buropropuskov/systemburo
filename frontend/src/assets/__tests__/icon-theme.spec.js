...
describe('icon-theme.css', () => {
  ...
  it('числа в шапке файла не расходятся с делом', () => {
    ...
    const [, total] = CSS.match(/(\d+) иконк\S* лежа\S* в PNG/);
    const [, mono] = CSS.match(/(\d+) из них ОДНОТОННЫЕ/);
    expect(Number(total), 'в шапке названо не то число растровых файлов').toBe(files.length);
    expect(Number(mono), 'в шапке названо не то число однотонных').toBe(monoNames.length);
  });
