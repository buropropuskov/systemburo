<template>
  <div class="action-bar-root">
    ...
    <div class="action-buttons-wrapper">
      <!-- Режим центра заявок -->
      <!-- Якорь туров согласующего и принимающего: набор кнопок внутри зависит от
           роли и стадии заявки, поэтому шаги смотрят на сам ряд, а не на кнопку -
           иначе шаг про решение по заявке молча пропадал бы, стоило заявке ждать
           согласующих (у принимающего вместо «Принять» стоит надпись ожидания). -->
      <div
        v-if="mode === 'center'"
        class="action-buttons"
        data-testid="ob-detail-actions"
      >
...
/* Мобилка: кнопки/статусы согласования в ОДНУ строку, не переносить (detail 2).
   ...
   Блок стоит ПОСЛЕДНИМ в файле намеренно. Медиа-запрос специфичности не добавляет,
   поэтому одноклассовое правило отсюда проигрывает такому же правилу, объявленному
   ниже по файлу: компактный padding кнопок решения так и не применялся, пара
   «Согласовать и принять» + «Отказать» занимала 348px при 330 доступных на 360 и
   вылезала за край шапки (замерено в браузере). Новые мобильные правила дописывать
   сюда же, а не поднимать блок обратно к базовым. */
@media (max-width: 768px) {
    ...
    /* min-width снимаем: пилюли идут по содержимому, как соседние «Дополнить»/
       «Отозвать» в шапке детали. Одного padding хватило бы впритык (328px при 330
       доступных на 360), а базовые 120px держали «Отказать» на 23px шире её текста -
       без них у пары остаётся 25px запаса на разные шрифты и длину подписи. */
    .confirm-btn,
    .reject-btn,
    .accept-btn {
        padding: 8px 14px;
    ...
    /* Отзыв решения стоит в ряду с "Принять"/"Отказать" (#1550) - без своей ширины,
       иначе тройка кнопок вылезает за 390. Два класса в селекторе оставлены
       сознательно: min-width: 140px приходит от .subtle-btn, и перебить его надо
       по специфичности, а не по месту правила в файле. */
