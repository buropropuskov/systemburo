...
/* Ключевая колонка машины ужимается до 114px (номер+марка, узкий контейнер) -
   бейджу статуса дополнения ("На согласовании" и длиннее) там не хватает
   места, а Badge.vue держит текст в один ряд (white-space: nowrap): пилюля
   зажималась по max-width, а буквы вылезали наружу без фона под ними
   (замерено в браузере - фон 114px, текст до 143px). Разрешаем перенос текста
   внутри самого бейджа - фон растёт вместе с ним, вместо того чтобы обрезать
   или ронять содержимое за свои границы. Три класса нужны, чтобы перебить
   scoped white-space: nowrap из Badge.vue по специфичности. */
