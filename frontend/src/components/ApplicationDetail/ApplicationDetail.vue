...
@container (max-width: 1040px) {
    .participants-btn {
        ...
        /* Тут "Получатели" уже кружок с иконкой, а не пилюля с подписью - полные -10px
           пилюльного режима смотрелись слишком тесно рядом с "Переслать", но обычный
           20px ряда всё ещё читался разреженно (замерено в браузере на этой самой
           заявке - 1440, ряд шапки сжат до кружка контейнерным запросом). */
        margin-left: -6px;
        border-radius: 50%;
        justify-content: center;
    }
...
/* Ряд действий заявки (в т.ч. решение по раунду дополнения) - на своей строке,
   ПОД бейджем "+ Дополнение №N на согласовании": .detail-header-actions - flex-ряд
   с переносом, и без явного flex-basis панель на широком экране вставала рядом с
   бейджем, а не под ним (владелец: "размести кнопки под шапкой"). Правило работает
   и без бейджа - панель здесь единственный ребёнок, flex-basis:100% просто отдаёт
   ей всю ширину ряда, а прижатие вправо остаётся на её собственном align-items. */
