...
describe('ApplicationAttachmentDetail — метка не наезжает на соседние поля', () => {
  ...
  // Ключевая колонка машины на десктопе сжимается до 114px (номер+марка, узкая таблица),
  // а Badge.vue держит текст в один ряд (white-space: nowrap) - пилюля "На согласовании"
  // зажималась по max-width: 100%, и буквы вылезали за фон пилюли без подложки под ними
  // (владелец: не помещается, обрезается). Замок держит перенос текста внутри бейджа -
  // фон растёт вместе со строками вместо того чтобы ронять их за свою границу.
