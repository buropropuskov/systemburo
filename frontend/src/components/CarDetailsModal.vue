    ...
    async exportHistory() {
      ...
      try {
        ...
        await downloadExcelSheet({
          sheetName: 'История въездов/выездов',
          header: ['Дата и время', 'Пользователь', 'Действие', 'Комментарий'],
          rows: this.history.map(item => [
        ...
        }, `История_автомобиля_${this.car.car_number}_${stamp.replace(/[.: ]/g, '-')}.xlsx`);
      } catch {
...
/* Рамки у фотографии нет: секция уже обведена и обрезает содержимое по своему
   скруглению - вторая линия внутри читается как двойная рамка. */
.photo-container {
  position: relative;
  width: 100%;
  height: 200px;
