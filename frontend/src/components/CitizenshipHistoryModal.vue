  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="modal-overlay"
        data-testid="citizenship-history-modal"
        @mousedown="onOverlayMousedown"
        @mouseup="onOverlayMouseup"
      >
        <div
          class="citizenship-history-modal"
          @mousedown.stop
        >
          <div class="modal-header">
            <h3>История гражданства «{{ citizenship.name }}»</h3>
...
<script>
import { ref } from 'vue';
import { getCitizenshipHistory } from '@/api/citizenships';
...
const ACTION_TEXTS = {
  created: 'Гражданство создано',
  updated: 'Изменены данные гражданства',
  archived: 'Гражданство архивировано',
  restored: 'Гражданство восстановлено из архива',
};
...
// Порядок и подписи полей гражданства для diff в истории (см. backend buildCitizenshipUpdateDetails).
const FIELD_LABELS = {
  name: 'Наименование',
  icon: 'Иконка',
  is_default: 'По умолчанию',
  patent_required: 'Требуется патент',
};
const FIELD_ORDER = ['name', 'icon', 'is_default', 'patent_required'];

// Поля-флаги: diff приходит как bool old/new, в timeline выводим как Да/Нет.
const BOOL_FIELDS = new Set(['is_default', 'patent_required']);
...
export default {
  name: 'CitizenshipHistoryModal',
  components: { LoaderSpinner, AppIcon },
  props: {
    citizenship: { type: Object, required: true },
  ...
  computed: {
    ...
    safeCitizenshipName() {
      const name = this.citizenship.name || `id_${this.citizenship.id}`;
  ...
  methods: {
    ...
    async loadHistory() {
      this.loading = true;
      try {
        const data = await getCitizenshipHistory(this.citizenship.id);
        this.history = Array.isArray(data) ? data : [];
      } catch (error) {
        console.error('Error loading citizenship history:', error);
        useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю гражданства', type: 'error' });
      } finally {
        this.loading = false;
      }
    },
    ...
    /**
     * Структурированные изменения для timeline. created - исходное имя;
     * updated - diff {old, new} по каждому изменённому полю (флаги выводятся
     * как Да/Нет); архив/восстановление деталей не имеют.
     */
    getChanges(item) {
      ...
      if (item.action_type === 'created') {
        if (!d.name) return [];
        return [{ label: FIELD_LABELS.name, kind: 'single', value: d.name }];
      }
    ...
    async exportToExcel() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet(`Istoriya_${this.safeCitizenshipName}`);
        ...
        a.download = `Istoriya_grazhdanstva_${this.safeCitizenshipName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`;
        a.href = url;
        a.click();
        window.URL.revokeObjectURL(url);
      } catch (error) {
        console.error('Error exporting citizenship history to Excel:', error);
...
.modal-fade-enter-active .citizenship-history-modal {
  animation: modal-scale-in 0.25s ease;
}

.modal-fade-leave-active .citizenship-history-modal {
