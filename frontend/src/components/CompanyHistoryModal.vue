  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="modal-overlay"
        data-testid="company-history-modal"
        @mousedown="onOverlayMousedown"
        @mouseup="onOverlayMouseup"
      >
        <div
          class="company-history-modal"
          @mousedown.stop
        >
          <div class="modal-header">
            <h3>История компании «{{ company.name }}»</h3>
...
export default {
  ...
  computed: {
    ...
    safeName() {
      const name = this.company.name || `id_${this.company.id}`;
  ...
  methods: {
    ...
    handleClickOutside(event) {
      ...
      const select = document.querySelector('[data-testid="company-history-modal"] .custom-select');
    ...
    async loadHistory() {
      this.loading = true;
      try {
        const response = await apiRequest(`/companies/${this.company.id}/history`);
        if (response.ok) {
          const data = await response.json();
          this.history = Array.isArray(data) ? data : [];
        } else {
          useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю компании', type: 'error' });
        }
      } catch (error) {
    ...
    getActionComment(item) {
      ...
      // Ключ type появился вместе с типом компании (#1046); у старых записей
    ...
    async exportToExcel() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet(`Istoriya_${this.safeName}`);
        ...
        a.download = `Istoriya_kompanii_${this.safeName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`;
        a.href = url;
        a.click();
...
.modal-fade-enter-active .company-history-modal {
  animation: modal-scale-in 0.25s ease;
}

.modal-fade-leave-active .company-history-modal {
