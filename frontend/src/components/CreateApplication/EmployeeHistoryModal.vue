...
export default {
  ...
  setup(_, { emit }) {
    ...
    // Bottom-sheet на мобилке: свайп вниз за ползунок закрывает (как CarHistoryModal).
  ...
  methods: {
    ...
    getActionClass(actionType) {
      const classes = {
        ...
        // Групповые операции над таблицами проходной (#1194 S2/S6).
    ...
    async exportToExcel() {
      ...
      try {
        await downloadExcelSheet({
          sheetName: `Istoriya_${this.safeFileName}`,
          header: [
            'Дата и время', 'Пользователь', 'Действие', 'Комментарий', 'Тип действия',
            'Старое значение', 'Новое значение', 'Поле', 'ID сотрудника', 'Место',
          ],
          ...
          widths: [25, 40, 30, 60, 20, 25, 25, 20, 15, 30],
          info: [
            ['Отчёт сформировал:', this.currentUserDisplayName],
            ['Дата формирования:', this.formattedCurrentDateTime],
          ],
          outerBorder: true,
        }, `Istoriya_sotrudnika_${this.safeFileName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`);
      } catch {
...
@keyframes employeeHistorySlideUp {
...
@media (max-width: 768px) {
  /* Bottom-sheet: окно выезжает снизу, свайп вниз за ползунок закрывает (#1097 R4-9). */
  .modal-overlay {
    padding: 0;
  ...
  .employee-history-modal.is-dragging {
    transition: none;
  }
  ...
  .modal-fade-enter-active .employee-history-modal {
    animation: employeeHistorySlideUp 0.3s ease-out;
  }
  ...
  .modal-fade-leave-to .employee-history-modal {
