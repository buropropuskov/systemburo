  ...
  methods: {
    async loadHistory() {
      ...
      try {
        ...
        const url = new URL('http://localhost:8080/employees/history/unified');
        url.searchParams.append('last_name', this.lastName || '');
        url.searchParams.append('first_name', this.firstName || '');
        if (this.middleName) url.searchParams.append('middle_name', this.middleName);

        console.log('Loading unified employee history from:', url.toString());
        ...
        if (response.ok) {
          ...
          console.log('Unified employee history loaded:', this.history);
        } else {
