<template>
  <BaseModal
    :show="show"
    :title="`История проходов: ${tableName}`"
    width="800px"
    @close="$emit('close')"
  >
...
export default {
  ...
  computed: {
    ...
    uniqueEmployees() {
      const map = new Map()
      for (const item of this.history) {
        if (item.employee_id && !map.has(item.employee_id)) {
          const name = [item.employee_last_name, item.employee_first_name].filter(Boolean).join(' ')
          map.set(item.employee_id, { id: item.employee_id, name })
        }
      }
      return Array.from(map.values())
    },
    ...
    filteredHistory() {
      ...
      if (this.searchText) {
        ...
        result = result.filter((item) => {
          ...
          const empName = [item.employee_last_name, item.employee_first_name].filter(Boolean).join(' ').toLowerCase()
          return (
      ...
      if (this.filterEmployee) {
        result = result.filter((item) => item.employee_id === this.filterEmployee)
      }
  ...
  methods: {
    async fetchHistory() {
      ...
      try {
        const response = await apiRequest(`/employees/history/table/${this.tableId}`)
        if (!response.ok) {
    ...
    async exportToExcel() {
      ...
      for (const item of this.filteredHistory) {
        const empName = [item.employee_last_name, item.employee_first_name].filter(Boolean).join(' ')
        sheet.addRow({
      ...
      link.href = url
      link.download = `История_таблица_${this.tableName}.xlsx`
      link.click()
