...
describe('CreateApplication — предупреждение о сроке машины «По факту»', () => {
  ...
  it('плашка об ошибке дат и подсказка о сроке живут рядом, не мешая друг другу', () => {
    ...
    // Подсказка стоит под датами и не участвует в потоке. Три забракованных
    // варианта позади: резерв высоты в потоке (блок дат подрастал при каждом
    // появлении подсказки), уход вбок (подсказка оказывалась «где-то справа
    // внизу») и якорь под чекбоксом (слишком низко).
    expect(разметка, '--below опускает пузырёк под блок дат').toContain('hint-anchor--below');
    expect(разметка, '--float уводит якорь из потока, иначе флекс добавит строку с зазором')
      .toContain('hint-anchor--float');
