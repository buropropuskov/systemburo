...
describe('CreateApplication — предупреждение о сроке машины «По факту»', () => {
  ...
  it('плашка об ошибке дат и подсказка о сроке живут рядом, не мешая друг другу', () => {
    ...
    // Подсказка уходит вбок. Ни --below, ни резерв высоты: снизу пузырёк ложился
    // на «однодневную заявку», а резерв раздувал блок дат на свою высоту при
    // каждом появлении подсказки - оба варианта владелец забраковал на стенде.
    expect(разметка, 'подсказка уходит вбок').toContain('hint-anchor--aside');
    expect(разметка, 'вниз её больше не разворачивают').not.toContain('hint-anchor--below');
