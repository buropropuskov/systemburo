...
describe('CreateApplication — предупреждение о сроке машины «По факту»', () => {
  ...
  it('плашка об ошибке дат и подсказка о сроке живут рядом, не мешая друг другу', () => {
    ...
    // Пузырёк потоку не виден, поэтому у якоря два условия: он идёт ПОСЛЕ
    // «однодневной заявки» и держит себе высоту. Иначе подсказка ложится на
    // подпись чекбокса - ровно тот дефект, из-за которого её и разворачивали вниз.
    expect(разметка, 'якорь резервирует место под пузырёк').toContain('hint-anchor--reserve');
    expect(
      разметка.indexOf('period-hint-anchor') > разметка.indexOf('однодневная заявка'),
      'якорь должен стоять ниже чекбокса однодневной заявки',
    ).toBe(true);
  });
});
