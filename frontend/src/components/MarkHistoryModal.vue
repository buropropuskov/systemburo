    ...
    async exportToExcel() {
      ...
      try {
        await downloadDirectoryHistory({
          sheetName: `Istoriya_${this.safeMarkName}`,
          filename: `Istoriya_marki_${this.safeMarkName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`,
...
.modal-fade-enter-active .mark-history-modal {
  animation: modal-scale-in 0.25s ease;
}

.modal-fade-leave-active .mark-history-modal {
