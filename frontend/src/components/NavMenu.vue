...
/* Невысокие десктоп-окна: полный набор разделов (+«Оформление») не помещается по
   высоте и уходит в прокрутку. Слегка уплотняем вертикаль пунктов и подписей
   секций, чтобы меню влезало целиком без скролл-бара (проверено: помещается до
   ~720px). Только десктоп-рельс (min-width:769) - у мобильного drawer <=768 свои
   тач-таргеты 48px, их не трогаем; телефоны тоже ниже 880 по высоте и попали бы
   под правило без гарда ширины. Порог 880 берёт типовые ноуты (полезная высота
   ~740-800 при экране 900-864). Высокие экраны (>880) - вид без изменений.
   Правила НЕ заскоуплены на .expanded намеренно: свёрнутый рельс уплотняется так
   же, иначе высота пунктов прыгала бы при hover-разворачивании; трогаем только
   вертикаль (min-height пункта чуть ниже + меньшие отступы подписей секций).
   Зазор между пунктами (margin 2px) и центрирование иконки (padding-left) НЕ
