...
export default {
  ...
  async mounted() {
    ...
    // Переход на рельс (>=1367) закрывает drawer: иначе он завис бы
    ...
    if (typeof window !== 'undefined' && typeof window.matchMedia === 'function') {
      this._desktopMql = window.matchMedia('(min-width: 1367px) and (hover: hover)');
...
/* «Сообщить о проблеме» в drawer'е (W3.3) - только когда меню в drawer. */
.nav-menu__feedback {
  display: none;
}
...
/* Невысокие десктоп-окна: полный набор разделов (+«Оформление») не помещается по
   ...
   прилипают к разделителям). Только десктоп-рельс (min-width:1367): у планшетного
   drawer свои тач-таргеты 48px, а телефоны тоже ниже 880 по высоте и попали
   ...
   пунктов. */
@media (min-width: 1367px) and (max-height: 880px) and (hover: hover) {
  .nav-item {
    min-height: 36px;
  }
}
