  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="modal-overlay"
        data-testid="org-history-modal"
        @mousedown="onOverlayMousedown"
        @mouseup="onOverlayMouseup"
      >
        <div
          class="org-history-modal"
          @mousedown.stop
        >
          <div class="modal-header">
            <h3>История организации «{{ organization.name }}»</h3>
...
export default {
  ...
  computed: {
    ...
    safeName() {
      const name = this.organization.name || `id_${this.organization.id}`;
  ...
  methods: {
    ...
    handleClickOutside(event) {
      ...
      const select = document.querySelector('[data-testid="org-history-modal"] .custom-select');
    ...
    async loadHistory() {
      this.loading = true;
      try {
        const response = await apiRequest(`/organizations/${this.organization.id}/history`);
        if (response.ok) {
          const data = await response.json();
          this.history = Array.isArray(data) ? data : [];
        } else {
          useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю организации', type: 'error' });
        }
      } catch (error) {
    ...
    async exportToExcel() {
      ...
      try {
        ...
        a.download = `Istoriya_organizacii_${this.safeName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`;
        a.href = url;
        a.click();
...
.modal-fade-enter-active .org-history-modal {
  animation: modal-scale-in 0.25s ease;
}

.modal-fade-leave-active .org-history-modal {
