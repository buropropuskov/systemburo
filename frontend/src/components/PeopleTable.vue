  ...
  methods: {
    ...
    async fetchPeopleData() {
      try {
        ...
        const tableRes = await fetch(`http://localhost:8080/system-tables/name/${this.tableName}`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
        ...
        const employeesRes = await fetch(`http://localhost:8080/employees/active-for-table/${table.id}`, {
          headers: { "Authorization": `Bearer ${token}` }
        });
    ...
    async fetchEmployeesStatus() {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch("http://localhost:8080/employees/history/current-status", {
          headers: { "Authorization": `Bearer ${token}` }
        });
        if (response.ok) {
    ...
    async handleEntryExit(item, type) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/employees/${item.id}/territory-status`, {
          method: "PUT",
          headers: {
    ...
    async actuallyDeleteItem(item, originalItem, itemIndex) {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`http://localhost:8080/employees/${item.id}/deactivate`, {
          method: "PUT",
          headers: {
            "Authorization": `Bearer ${token}`,
            "Content-Type": "application/json"
