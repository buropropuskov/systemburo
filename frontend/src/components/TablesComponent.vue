...
export default {
    ...
    computed: {
        ...
        // Блок "по факту" существует только для машин: строку с номером "по факту"
        // заявитель создаёт тумблером в форме транспорта, у сотрудников такого тумблера
        // нет. У таблицы людей блок всегда выходил пустым и читался как "заявок нет",
        // хотя заявки были (#2019). Флажок у таких таблиц больше не показывается в
        // конструкторе, но у заведённых раньше он мог остаться включённым - гасим здесь.
        showFactTable() {
            if (this.tableData?.table?.table_type !== 'cars') return false;
