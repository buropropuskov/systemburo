...
@media (max-width: 768px) {
  /* Шапка закреплена сверху при скролле - "Подать заявку" и колокольчик всегда доступны.
     ...
     fixed + top по visualViewport (--app-vvt, App.vue): шапка следует за реально видимой
     областью и не «дёргается» на ~5px при сворачивании адресной строки Яндекса (sticky на
     скролле документа снапал). Высота компенсируется padding-top у #main-content. */
  .header {
    position: fixed;
    top: var(--app-vvt, 0);
    left: 0;
