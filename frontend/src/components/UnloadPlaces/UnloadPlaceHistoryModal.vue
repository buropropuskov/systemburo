  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="modal-overlay"
        data-testid="unload-place-history-modal"
        @mousedown="onOverlayMousedown"
        @mouseup="onOverlayMouseup"
      >
        <div
          class="up-history-modal"
          @mousedown.stop
        >
          <div class="modal-header">
            <h3>История места разгрузки «{{ unloadPlace.name }}»</h3>
...
const ACTION_TEXTS = {
  created: 'Место разгрузки создано',
  renamed: 'Место разгрузки переименовано',
  archived: 'Место разгрузки архивировано',
  restored: 'Место разгрузки восстановлено из архива',
};
...
export default {
  name: 'UnloadPlaceHistoryModal',
  components: { LoaderSpinner, AppIcon },
  props: {
    unloadPlace: { type: Object, required: true },
    currentUserName: { type: String, default: '' },
  },
  emits: ['close'],
  setup(_, { emit }) {
    // Анимация закрытия: close() прячет модалку (v-if -> false), а @after-leave
    // эмитит close родителю уже после проигрыша leave-перехода.
    const visible = ref(true);
    const close = () => { visible.value = false; };
    const onAfterLeave = () => emit('close');
    const { onOverlayMousedown, onOverlayMouseup } = useOverlayClose(close);
    return { visible, close, onAfterLeave, onOverlayMousedown, onOverlayMouseup };
  },
  data() {
    return {
      loading: false,
  ...
  computed: {
    ...
    safeName() {
      const name = this.unloadPlace.name || `id_${this.unloadPlace.id}`;
  ...
  methods: {
    ...
    async loadHistory() {
      this.loading = true;
      try {
        const response = await apiRequest(`/unload-places/${this.unloadPlace.id}/history`, {});
        if (response.ok) {
          const data = await response.json();
          const arr = Array.isArray(data) ? data : [];
          // Бэк (#452) отдаёт actor_user_id/actor_name; таймлайн-эталон читает
          // user_id/user_name - нормализуем при загрузке, дальше логика 1-в-1.
          this.history = arr.map((item) => ({
            ...item,
            user_id: item.actor_user_id ?? null,
            user_name: item.actor_name || '',
          }));
        } else {
          useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю места разгрузки', type: 'error' });
        }
      } catch (error) {
        console.error('Error loading unload place history:', error);
        useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю места разгрузки', type: 'error' });
      } finally {
        this.loading = false;
      }
    ...
    /**
     * Читаемые детали для timeline. Бэк пишет details={name} на создание и
     * переименование, на архив/восстановление - null. Старого имени в аудите
     * нет, поэтому diff невозможен - показываем только текущее имя.
     */
    getActionComment(item) {
      const d = item.details || {};
      if (item.action_type === 'created') {
        return d.name ? `Название: ${d.name}` : '';
      }
      if (item.action_type === 'renamed') {
        return d.name ? `Новое название: ${d.name}` : '';
      }
      return '';
    },
    ...
    async exportToExcel() {
      ...
      try {
        ...
        // Имя листа Excel ограничено 31 символом - режем по запасу.
        const worksheet = workbook.addWorksheet(`Istoriya_${this.safeName}`.slice(0, 31));
        ...
        a.download = `Istoriya_mesta_razgruzki_${this.safeName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`;
        a.href = url;
        a.click();
        window.URL.revokeObjectURL(url);
      } catch (error) {
        console.error('Error exporting unload place history to Excel:', error);
...
/* Анимация открытия/закрытия: фейд оверлея + slide-up контента (эталон ui-modals). */
...
.modal-fade-enter-active .up-history-modal {
  transition: transform 0.25s ease;
}

.modal-fade-enter-from .up-history-modal {
  transform: translateY(20px);
}

.modal-fade-leave-active .up-history-modal {
  transition: transform 0.2s ease;
}

.modal-fade-leave-to .up-history-modal {
  transform: translateY(20px);
}
