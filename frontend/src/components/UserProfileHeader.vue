        ...
        <div class="user-detail" v-if="email" @click="copyEmail">
          ...
          >
            <transition name="fade" mode="out-in">
              <!-- Состояние до копирования: иконка + email -->
              ...
              <!-- Состояние после копирования: только текст -->
  ...
  methods: {
    updateBadgeWidths() {
      ...
      if (this.$refs.emailBadge && this.email) {
        ...
        // Ширина исходного блока (иконка + email)
        const originalWidth = clone.offsetWidth;
        
        // Ширина блока с сообщением о копировании (без иконки)
        const contentDiv = clone.querySelector('.badge-content');
        if (contentDiv) {
          // Удаляем иконку, оставляем только текст
