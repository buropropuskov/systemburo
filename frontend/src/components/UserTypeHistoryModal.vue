  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="visible"
        class="modal-overlay"
        data-testid="user-type-history-modal"
        @mousedown="onOverlayMousedown"
        @mouseup="onOverlayMouseup"
      >
        <div
          class="user-type-history-modal"
          @mousedown.stop
        >
          <div class="modal-header">
            <h3>История типа «{{ userType.name }}»</h3>
...
// Читаемые лейблы для полей в details.
const FIELD_LABELS = {
  name: 'Наименование',
  code: 'Системное имя',
};
...
export default {
  ...
  computed: {
    ...
    safeTypeName() {
      const name = this.userType.name || `id_${this.userType.id}`;
  ...
  methods: {
    ...
    handleClickOutside(event) {
      // Под Teleport this.$el - якорный comment без querySelector, ищем от body.
      const select = document.querySelector('[data-testid="user-type-history-modal"] .custom-select');
    ...
    async loadHistory() {
      this.loading = true;
      try {
        const response = await apiRequest(`/user-types-management/${this.userType.id}/history`);
        if (response.ok) {
          const data = await response.json();
          this.history = Array.isArray(data) ? data : [];
        } else {
          useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю типа', type: 'error' });
        }
      } catch (error) {
        console.error('Error loading user type history:', error);
        useDeletionsStore().notify({ prefix: 'Не удалось загрузить ', bold: 'историю типа', type: 'error' });
      } finally {
        this.loading = false;
      }
    ...
    /**
     * Читаемые детали для timeline. Для типов details - простые строки name/code.
     */
    getActionComment(item) {
      ...
      switch (item.action_type) {
        ...
        case 'deleted': {
          const parts = [];
          if (d.name) parts.push(`${FIELD_LABELS.name}: ${d.name}`);
          if (d.code) parts.push(`${FIELD_LABELS.code}: ${d.code}`);
          return parts.join(' / ');
        }
        case 'renamed':
    ...
    async exportToExcel() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet(`Istoriya_${this.safeTypeName}`);
        ...
        a.download = `Istoriya_tipa_${this.safeTypeName}_${this.formattedCurrentDateTime.replace(/[.:,]/g, '-')}.xlsx`;
        a.href = url;
        a.click();
...
.modal-fade-enter-active .user-type-history-modal {
  animation: modal-scale-in 0.25s ease;
}

.modal-fade-leave-active .user-type-history-modal {
