...
describe('UserApplications — чип "Обновления" (#1349 срез 4)', () => {
  ...
  it('пункты фильтра читаются целиком, а меню не уже их (#2339)', async () => {
    ...
    // menuMinWidth считается только в режиме телепорта: без него проп ничего не делал,
    // и «Заявки организации» резалось многоточием при заданной ширине (#2339).
    expect(dd.props('teleport'), 'без телепорта заданная ширина меню не применяется').toBe(true);
  });
