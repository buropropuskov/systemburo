...
async function prepareStepInner(globalIndex) {
  ...
  // Необязательный шаг ждём коротко (700 мс): к этому моменту форма отрисована
  // на предыдущем шаге, и отсутствие цели значит «её тут нет» - иначе подряд
  // идущие кнопки карточки давали по 4 с на каждую и читались как «нажал Далее,
  // а ничего не происходит» (#1771). Полное ожидание положено, когда на шаге
  // что-то раскрывали, меняли бланк, шаг обязателен или объявил `waitsData` -
  // последнее про цели, приезжающие отдельным запросом после открытия узла
  // (состав вложения, раунды дополнения, бланки): им 700 мс не хватало, и они
  // выпадали молча, унося половину тура принимающего (#2584).
  ...
  const needsLongWait = revealed || attachmentChanged || !step.optional || step.waitsData;
