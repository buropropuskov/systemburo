...
describe('замок против смешения ролей', () => {
  ...
  // «Согласовать и принять» в список НЕ входит: так подписана кнопка принимающего
  ...
  it('шаг с ожиданием данных знает свой контейнер, иначе держит тур', () => {
    // Ждать цель вслепую - 2,5 с на каждый неприменимый шаг; три подряд давали
    // 8 с паузы и читались как зависший тур (#2590). Контейнер отвечает на вопрос
    // «данные уже здесь?»: пришли, а цели нет - значит её тут и не будет.
    // acc-center-blacklist ждёт вслепую намеренно: тег приезжает со строками
    // списка, а контейнер списка стоит и до данных - ответа «данные здесь?» он не
    // даёт, и на коротком ожидании шаг выпадал.
    const безКонтейнера = ['acc-detail-attachments', 'acc-center-blacklist'];
    const без = acceptOnboardingSteps
      .filter((s) => s.waitsData && !безКонтейнера.includes(s.id))
      .filter((s) => !s.dataReady)
      .map((s) => s.id);
    expect(без).toEqual([]);
  });

  it('шаги про содержимое карточки ждут данные, иначе выпадают молча', () => {
    // Состав вложения, чипы мест и раунды дополнения приезжают отдельным запросом
    // уже после открытия карточки. Без waitsData такому шагу достаётся 700 мс, и
    // он выбрасывается на живой заявке - так терялась половина тура (#2584).
    const DATA_ANCHORS = /^(attachment-|supplement-panel|ob-org-moderation|app-detail-button-download)/;
    const забыли = acceptOnboardingSteps
      .filter((s) => s.element)
      .filter((s) => DATA_ANCHORS.test(s.element.match(/^\[data-testid="([^"]+)"\]$/)[1]))
      .filter((s) => !s.waitsData)
      .map((s) => s.id);
