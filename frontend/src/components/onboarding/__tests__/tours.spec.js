 ...
 */
/**
 * Шаг про список уведомлений подсвечивает панель, которая у новичка почти всегда
 * пуста: там только заголовок, вкладки и надпись «Нет уведомлений». Прежде текст
 * рассказывал про свежие события и выделенные непрочитанные - то есть про то,
 * чего на экране нет, и владелец видел подсвеченную пустую коробку (замечание
 * 21.08). Текст обязан работать в обе стороны: и когда события есть, и когда нет.
 */
describe('шаг про список уведомлений честен на пустом списке', () => {
  const panels = allTourSteps().filter((s) => s.element === '[data-testid="ob-notifications-panel"]');

  it('такой шаг есть в турах, где разбирают колокольчик', () => {
    expect(panels.length).toBeGreaterThan(0);
  });

  it('текст объясняет пустой список, а не обещает события', () => {
    panels.forEach((step) => {
      expect(step.description, step.id).toMatch(/[Пп]уст/);
    });
  });

  it('названы вкладки, которые видно даже без единого события', () => {
    panels.forEach((step) => {
      expect(step.description, step.id).toContain('Непрочитанные');
    });
  });
});
