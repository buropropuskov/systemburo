...
export function buildSecurityFactSteps(route) {
  ...
  return [
    ...
    },
    {
      ...
      description:
        'Список «по факту» - те, кого в заявке не было: машину пропустили по месту, и запись осталась здесь.',
    },
    {
      // Отдельный шаг на кнопке ручного ввода: окно открывается только по
      // нажатию, и подсветить его самому туру нечем - показываем кнопку и
      // рассказываем, что будет дальше.
      id: 'sec-fact-pass',
      route,
      ...
      description:
        'При пропуске по факту открывается окно: формат номера и номер Т/С обязательны, марка - по желанию. Кнопка «Пропустить» сохраняет запись; до этого отметка не ставится.',
    },
    {
      id: 'sec-fact-report',
      route,
