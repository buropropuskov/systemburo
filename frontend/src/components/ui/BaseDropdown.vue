...
/*
 * Ширина меню В ПОТОКЕ (без телепорта): кнопка задаёт нижнюю границу, дальше меню
 * растёт по содержимому. Кнопку фильтра зажимает колонка таблицы, и при жёстком
 * width: 100% пункты резались многоточием - в справочнике организаций из шести
 * пунктов обрезались пять при ширине меню 117px.
 *
 * Селектор ведёт ОТ КОНТЕЙНЕРА намеренно. Телепортнутое меню лежит прямым потомком
 * body, и там 100% - это ширина всей страницы: правило без контейнера растянуло меню
 * фильтров Центра на 1600px и вынесло за правый край. В том режиме ширину считает
 * updateMenuPosition и задаёт инлайном, вместе со сдвигом от края.
 */
.base-dropdown .base-dropdown__menu {
  min-width: 100%;
  width: max-content;
}
...
.base-dropdown__menu {
  ...
  top: calc(100% + 5px);
  left: 0;
  /* Потолок общий для обоих режимов - меню не должно уезжать за край экрана. */
