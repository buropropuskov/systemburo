...
export const ADMIN_GROUPS = [
  {
    ...
    items: [
      ...
      { label: 'Доступ к перс. данным', icon: 'pd-audit', path: '/admin/pd-audit', permission: 'page.admin.pd_audit' },
      { label: 'Сведения о человеке', icon: 'pd-subject', path: '/admin/pd-subject', permission: 'page.admin.pd_subject' },
...
export const MAIN_SECTIONS = [
  ...
  {
    // Раздел живёт в поиске и открывается прямой ссылкой; пункта в рельсе у него нет
    // намеренно (#2305): доступность обеспечена, а место в меню того не стоило.
    label: 'Отчёты',
    icon: 'analytics',
    path: '/analytics',
    query: { tab: 'reports' },
