...
/**
 * Транслитерация (#2414). Одно и то же название пишут буквами двух алфавитов, а человек
 * ищет тем, которое знает. Клиентское ядро обязано вести себя как серверное
 * (normalize.Translit): две схемы записи, схлопывание удвоений, порог длины.
 */
describe('searchVariants — транслитерация', () => {
  const находит = (haystack, query) => matchesSearch(haystack, buildSearchVariants(query));

  it('кириллический запрос находит латинскую запись', () => {
    // Ради этого случая задача и заводилась: «Траттория» не находила «La Trattoria».
    expect(находит('La Trattoria', 'траттория')).toBe(true);
    expect(находит('Sergeev Ivan', 'сергеев')).toBe(true);
  });

  it('латинский запрос находит кириллическую запись', () => {
    expect(находит('Сергеев Иван', 'sergeev')).toBe(true);
  });

  it('вывесочное написание учитывается наравне с документным', () => {
    // «я» в конце пишут и как «ya», и как «a» - нужны обе схемы.
    expect(находит('Trattoriya', 'траттория')).toBe(true);
    expect(находит('Trattoria', 'траттория')).toBe(true);
  });

  it('потерянное удвоение находит запись с удвоением и наоборот', () => {
    expect(находит('La Trattoria', 'тратория')).toBe(true);
    expect(находит('Alleya', 'алея')).toBe(true);
  });

  it('короткий запрос не транслитерируется - иначе тянет случайные подстроки', () => {
    // «рга» в латинице даёт «rga», а это кусок слова Organization: поиск начинал
    // выдавать все записи организации подряд.
    expect(находит('Test Organization', 'рга')).toBe(false);
    expect(buildSearchVariants('ким').length, 'три буквы - только раскладка').toBeLessThanOrEqual(3);
  });

  it('вариантов остаётся немного - каждый это отдельная проверка на строку', () => {
    for (const q of ['траттория', 'александрович', 'la trattoria', 'sergeev']) {
      expect(buildSearchVariants(q).length, `слишком много вариантов для «${q}»`).toBeLessThanOrEqual(12);
    }
  });
});
