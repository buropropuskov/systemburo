...
/**
 * Подсказка у полей даты: сначала что сделать, потом до какого момента.
 *
 * «Укажите корректный срок» первым - подсказка стоит у полей, которые надо
 * поправить, и должна читаться как указание, а не как справка. Дата с временем
 * следом, чтобы не пришлось искать её в панели.
 */
export function byFactFieldHint(now = serverNow()) {
  return `Укажите корректный срок — по ${byFactDeadlineExact(now)}`;
}
