...
const DIRECT_FETCH_ALLOWED = {
  ...
  created: 'конец обучения: на время тура список подменялся примерной заявкой, и без перезапроса она оставалась висеть как настоящая (#2610)',
};
...
describe('Центр заявок: смена фильтра не дёргает шапку и список', () => {
  ...
  it('уходящая строка лежит под теми, кто занял её место', () => {
    // position: absolute кладёт уходящую в слой выше статических соседей, и
    // z-index: 0 на ней этого не отменяет. Слой задаётся тем, кто въезжает.
    // Правило со слоем - групповое: у move есть ещё правило с transition выше,
    // поэтому ищем блок, где селекторы перечислены через запятую.
    const layer = (transitions.match(/\.app-row-filter-move,[\s\S]{0,200}?\{[^}]*\}/) || [''])[0];

    expect(
      /app-row-filter-enter-active/.test(layer),
      'появившиеся строки не подняты слоем - уезжающая пройдёт поверх них',
    ).toBe(true);
    expect(
      /position:\s*relative/.test(layer) && /z-index:\s*[1-9]/.test(layer),
      'въезжающие строки не подняты слоем - уезжающая будет проходить поверх них',
    ).toBe(true);
  });

  it('слой поднимается только на время движения', () => {
    // Постоянный слой делает каждую строку стековым контекстом, и всплывающие
    // подсказки (.hint-anchor::after, z-index 60) перестают выходить за её пределы.
    // Замер на стенде: подсказка тега уходила под следующую строку, подсказки
    // отправителя и номера - под предыдущую. Жалоба владельца: «hint тегов
    // отображается под элементами».
    expect(
      /^\.applications-list\s*>\s*\*/m.test(transitions),
      'слой навешен на все строки разом - всплывающие подсказки уйдут под соседей',
    ).toBe(false);
  });
