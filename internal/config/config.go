...
type Config struct {
	...
	// AuditRetentionMonths - срок хранения истории сущностей (audit_log) при
	// автоматической уборке (#2355).
	//
	// Срок у группы audit был описан (три года), но применялся только вручную
	// командой server cleanup: автоматическая уборка его не трогала, и записи с ФИО
	// в пояснениях («Сотрудник Иванов Иван прошёл на территорию») копились бессрочно.
	//
	// По умолчанию 0 - выключено, как и у срока заявок: удаление истории оператор
	// назначает сам. Из уборки исключены записи корзины и последние отметки входа и
	// выхода - из них считается «последний выезд» в карточке.
	AuditRetentionMonths int `env:"AUDIT_RETENTION_MONTHS" envDefault:"0"`
