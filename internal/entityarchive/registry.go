...
func organizationNodes() []Node {
	return []Node{
		...
		// is_shared/is_system у шаблона на решение не влияют: FK каскадный независимо
		// от них, и раз снос всё равно сотрёт их, лучше видимо (узел, экспорт, счётчик),
		// чем невидимо. Если публично расшаренный шаблон пережившей организации нужен
		// другим - это отдельное решение о переносе владения ДО purge, не задача среза.
