...
//
// Основание обработки одно для всех строк: обеспечение пропускного режима на объекте.
// Ссылки на статьи закона в справку не идут - юридическую квалификацию даёт тот, кто
// готовит ответ, а не программа. Отметка уведомления работника и возражение против
// обработки печатаются рядом со сведениями: именно они меняют режим работы с записью.

// Основание обработки в тексте справки. Держится здесь, а не в шаблоне: строка уходит
// в официальный ответ, и менять её заодно с вёрсткой нельзя.
const subjectProcessingBasis = "Обеспечение пропускного режима на объекте"
