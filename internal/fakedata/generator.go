...
func Run(ctx context.Context, env *Env) error {
	for _, step := range Steps() {
		if err := step.Run(ctx, env); err != nil {
			// Партия остаётся открытой и с уже зарегистрированными записями: их надо
			// не потерять, иначе созданное до сбоя останется на стенде навсегда.
			// Удаляется такая партия тем же -purge, что и успешная.
