...
func (registriesStep) Run(ctx context.Context, env *Env) error {
	...
	// Ненайденный администратор -- тот же случай "тестов до сида", что resolveActor
	// уже документирует в generator.go: ронять шаг из-за пользователей, которых
	// ещё некому было завести, не стоит -- реестр остаётся пустым до первого
	// администратора на стенде.
	username := actorUsername(ctx, env.DB, env.ActorUserID)
	if username == "" {
		slog.Warn("наливка: реестры сотрудников и машин пропущены -- на стенде нет ни одного администратора, некому стать владельцем записей")
		return nil
	}
...
// actorUsername ищет username администратора стенда по его id.
func actorUsername(ctx context.Context, db *gorm.DB, actorID int) string {
	if actorID == 0 {
		return ""
	}
	var username string
	if err := db.WithContext(ctx).Raw(`SELECT username FROM users WHERE id = ?`, actorID).Scan(&username).Error; err != nil {
		slog.Error("наливка: не удалось определить username администратора стенда", "actor_id", actorID, "error", err)
		return ""
	}
	return username
}
