...
// TestCitizenships_History_BackfillLegacyIntoAudit проверяет финал #870 (срез F.1):
// читатель истории переведён на audit_log-only, а до-cutover строки замороженной
// citizenship_histories поднимаются в audit_log разовым BackfillAuditFromLegacy.
...
func TestCitizenships_History_BackfillLegacyIntoAudit(t *testing.T) {
	...
	legacy := models.CitizenshipHistory{
		CitizenshipID: id,
		ActionType:    models.CitizenshipActionUpdated,
		Details:       json.RawMessage(`{"name":{"old":"Узбекистан","new":"Республика Узбекистан"}}`),
	...
	require.NoError(t, db.Where("key = ?", "audit_backfilled:"+models.AuditEntityCitizenship).
	...
	require.NoError(t, db.Table("audit_log").
		Where("entity_type = ? AND entity_id = ? AND action = ?", models.AuditEntityCitizenship, id, models.CitizenshipActionUpdated).
		Count(&auditCount).Error)
	assert.Equal(t, int64(1), auditCount, "legacy updated перенесён в audit_log")
	require.NoError(t, db.Table("citizenship_histories").Where("citizenship_id = ?", id).Count(&legacyCount).Error)
	...
	// details перенесены verbatim.
	updated := hist[1]["details"].(map[string]interface{})
	name := updated["name"].(map[string]interface{})
	assert.Equal(t, "Узбекистан", name["old"])
	assert.Equal(t, "Республика Узбекистан", name["new"])
