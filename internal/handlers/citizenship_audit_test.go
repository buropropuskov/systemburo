...
// TestCitizenships_History_UnionLegacyAndAudit проверяет переходную модель #870:
// новые действия пишутся в audit_log, а строки из замороженной citizenship_histories
// по-прежнему видны в истории через union. Гарантирует "история та же" до backfill.
func TestCitizenships_History_UnionLegacyAndAudit(t *testing.T) {
	...
	require.NoError(t, db.Table("audit_log").Where("entity_type = ? AND entity_id = ?", models.AuditEntityCitizenship, id).Count(&auditCount).Error)
