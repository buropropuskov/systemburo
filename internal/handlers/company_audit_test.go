...
// TestCompanies_History_BackfillLegacyIntoAudit проверяет финал #870 (срез F.2):
// читатель истории переведён на audit_log-only, а до-cutover строки замороженной
// company_histories поднимаются в audit_log разовым BackfillAuditFromLegacy.
...
func TestCompanies_History_BackfillLegacyIntoAudit(t *testing.T) {
	...
	rec := testutil.POST(t, e, "/companies", `{"name":"Ауди Ко"}`, h)
	...
	legacy := models.CompanyHistory{
		CompanyID:  id,
		ActionType: models.CompanyActionRenamed,
	...
	rec = testutil.GET(t, e, fmt.Sprintf("/companies/%d/history", id), h)
	require.Equal(t, http.StatusOK, rec.Code)
	...
	require.NoError(t, db.Where("key = ?", "audit_backfilled:"+models.AuditEntityCompany).
	...
	require.NoError(t, db.Table("audit_log").
		Where("entity_type = ? AND entity_id = ? AND action = ?", models.AuditEntityCompany, id, models.CompanyActionRenamed).
		Count(&auditCount).Error)
	assert.Equal(t, int64(1), auditCount, "legacy renamed перенесён в audit_log")
	require.NoError(t, db.Table("company_histories").Where("company_id = ?", id).Count(&legacyCount).Error)
