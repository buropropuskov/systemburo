...
// actorUserID - автор действия для журнала, всегда из токена (#2443).
//
// Раньше отметку прохода писали от того, кого назвал клиент: `user_id` приходил в
// теле запроса. Из консоли браузера так можно было записать проход на чужую фамилию,
// а суточный отчёт по постам группируется ровно по этому полю. Поле в теле больше не
// разбирается (json:"-"), а хендлеры проставляют автора отсюда.
//
// Пустой id (запрос без токена сюда не доходит) отдаётся как nil - в журнале это
// «Система», а не пользователь с номером ноль.
