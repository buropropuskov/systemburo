...
// TestLicenseFormats_History_UnionLegacyAndAudit проверяет переходную модель #870:
// новые действия пишутся в audit_log, а строки из замороженной
// license_plate_format_histories по-прежнему видны в истории через union.
func TestLicenseFormats_History_UnionLegacyAndAudit(t *testing.T) {
	...
	require.NoError(t, db.Table("audit_log").
		Where("entity_type = ? AND entity_id = ?", models.AuditEntityLicensePlateFormat, id).
		Count(&auditCount).Error)
	assert.Equal(t, int64(1), auditCount, "created должен попасть в audit_log")
	require.NoError(t, db.Table("license_plate_format_histories").
		Where("format_id = ?", id).Count(&legacyCount).Error)
