...
// TestLicenseFormats_History_BackfillLegacyIntoAudit проверяет финал #870 (срез F.2):
// читатель истории переведён на audit_log-only, а до-cutover строки замороженной
// license_plate_format_histories поднимаются в audit_log разовым BackfillAuditFromLegacy.
...
func TestLicenseFormats_History_BackfillLegacyIntoAudit(t *testing.T) {
	...
	rec := testutil.POST(t, e, "/license-plate-formats",
		`{"name":"Аудит RU","country_code":"RU","is_default":false,"cells":[]}`, h)
	...
	legacy := models.LicensePlateFormatHistory{
		FormatID:   id,
		ActionType: models.LicensePlateFormatActionUpdated,
		Details:    json.RawMessage(`{"name":{"old":"Старое Имя","new":"Аудит RU"}}`),
		CreatedAt:  time.Now().Add(-time.Hour),
	}
	require.NoError(t, db.Create(&legacy).Error)

	// До backfill читатель видит только audit_log -> legacy-строка ещё невидима.
	rec = testutil.GET(t, e, fmt.Sprintf("/license-plate-formats/%d/history", id), h)
	require.Equal(t, http.StatusOK, rec.Code)
	...
	require.NoError(t, db.Where("key = ?", "audit_backfilled:"+models.AuditEntityLicensePlateFormat).
	...
	require.NoError(t, db.Table("audit_log").
		Where("entity_type = ? AND entity_id = ? AND action = ?", models.AuditEntityLicensePlateFormat, id, models.LicensePlateFormatActionUpdated).
		Count(&auditCount).Error)
	assert.Equal(t, int64(1), auditCount, "legacy updated перенесён в audit_log")
	require.NoError(t, db.Table("license_plate_format_histories").Where("format_id = ?", id).Count(&legacyCount).Error)
	...
	assert.Equal(t, "Старое Имя", name["old"])
	assert.Equal(t, "Аудит RU", name["new"])
