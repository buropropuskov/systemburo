...
func TestPassageRevert_LastExitIgnoresRevoked(t *testing.T) {
	...
	before, err := svc.GetCarsCurrentStatus(ctx, userID, services.FullElementScope())
	require.NoError(t, err)
	...
	after, err := svc.GetCarsCurrentStatus(ctx, userID, services.FullElementScope())
	require.NoError(t, err)
...
func TestPassageRevert_LastExitIgnoresRevokedForPeople(t *testing.T) {
	...
	before, err := svc.GetCurrentStatus(ctx, userID, services.FullElementScope())
	require.NoError(t, err)
	require.Len(t, before, 1)
	require.Equal(t, services.FormatUTCPtr(&wrong.CreatedAt), before[0].LastExitTime,
	...
	after, err := svc.GetCurrentStatus(ctx, userID, services.FullElementScope())
	require.NoError(t, err)
	require.Len(t, after, 1)
	assert.Equal(t, services.FormatUTCPtr(&real.CreatedAt), after[0].LastExitTime,
