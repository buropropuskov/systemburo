...
// TestUserTypes_History_BackfillLegacyIntoAudit проверяет финал #870 (срез F.2):
// читатель истории переведён на audit_log-only, а до-cutover строки замороженной
// user_type_histories поднимаются в audit_log разовым BackfillAuditFromLegacy.
...
func TestUserTypes_History_BackfillLegacyIntoAudit(t *testing.T) {
	...
	require.NoError(t, db.Where("key = ?", "audit_backfilled:"+models.AuditEntityUserType).
	...
	require.NoError(t, db.Table("audit_log").
		Where("entity_type = ? AND entity_id = ? AND action = ?", models.AuditEntityUserType, id, models.UserTypeActionRenamed).
