...
type UniqueEmployee struct {
	...
	PDConsentAt       *time.Time `json:"pd_consent_at"`
	PDConsentByUserID *int       `json:"pd_consent_by_user_id"`
	// Возражение субъекта против обработки его данных (#2361). Не отзыв согласия:
	// согласия он не давал, его уведомили, а обработка идёт по законному интересу
	// оператора. Возражать при этом человек вправе, и оператор обязан возражение
	// рассмотреть. Пока отметка стоит, персональные поля записи только читаются;
	// организация, привязка и места прохода остаются изменяемыми, иначе встало бы
	// администрирование справочников.
	//
	// Время и автора ставит СЕРВЕР, как и у отметки об уведомлении: из запроса
	// приходит только основание - откуда поступило обращение. Иначе датой возражения
	// можно было бы прикрыть задним числом уже сделанную правку.
	PDObjectionAt       *time.Time `gorm:"index" json:"pd_objection_at"`
	PDObjectionByUserID *int       `json:"pd_objection_by_user_id"`
