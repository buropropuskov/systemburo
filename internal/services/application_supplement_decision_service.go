...
func (s *applicationService) DecideSupplement(ctx context.Context, username string, applicationID, supplementID int, req SupplementDecisionRequest) (*SupplementDecisionResponse, error) {
	...
	if req.Action == supplementActionAccept {
		// Статус заявки перечитываем внутри транзакции, уже держа раунд: между гардом
		// принимающего и активацией заявку могли вывести из работы, и тогда её строки
		// погашены, а состав раунда встал бы на КПП поверх снятых пропусков.
		//
		// FOR UPDATE тут намеренно нет: обратный порядок захвата (заявка -> раунд) уже занят
		// cancelOpenSupplements на выводе из работы, и встречная блокировка дала бы дедлок.
