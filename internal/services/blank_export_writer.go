...
// PruneEmptyDirs подчищает каталоги, опустевшие после уборки файлов заявки, вверх
// до корня архива. Best-effort, как и подъём после переезда: непустой каталог просто
// останавливает подъём. Пустая папка с номером заявки персональных данных не несёт,
// но остаётся следом уничтоженной записи в дереве, которое смотрят глазами.
func (w *ArchiveWriter) PruneEmptyDirs(levels []string) {
	dir, err := w.Resolve(levels...)
	if err != nil {
		return
	}
	w.pruneEmptyParents(dir)
}
