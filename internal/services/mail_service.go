...
// адресату, и для писем о пароле это пароль - в базе, которая по всем прочим
// правилам хранит пароли только вычислением Argon2id. Пока письмо ждёт отправки,
// его тело шифрует models.EmailMessage.BeforeSave (#2351), но стирание после
// терминального статуса надёжнее: ни ждать суточную уборку (MAIL_MESSAGE_RETENTION_DAYS),
// ни зависеть от ключа шифрования не нужно - копии просто нет.
//
...
func (s *mailService) markFailure(ctx context.Context, row *models.EmailMessage, cause error) {
	...
	if attempts >= s.cfg.MailRetryAttempts {
		...
		// в письмах о пароле он содержит сам пароль (#2351).
		updates["body"] = ""
