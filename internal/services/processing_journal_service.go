...
// GetProcessingJournal возвращает последние события согласования/принятия за период
// [from, to] по времени убыванием, не более limit. limit<=0 -> дефолт, выше потолка
// -> потолок. Окно бьёт по времени САМОГО события (согласование/принятие), а не по
// дате подачи: лента показывает, что происходило в выбранный период.
func (s *statisticsService) GetProcessingJournal(ctx context.Context, from, to time.Time, limit int) ([]models.ProcessingJournalEntry, error) {
	if limit <= 0 {
	...
	// ввода); границы окна и лимит идут именованными параметрами.
	...
	query := fmt.Sprintf(`
		...
		ORDER BY j.occurred_at DESC, j.application_id DESC, j.role
		LIMIT @limit`,
	...
	if err := s.db.WithContext(ctx).Raw(query,
		map[string]any{"from": from, "to": to, "limit": limit}).Scan(&entries).Error; err != nil {
		return nil, fmt.Errorf("statistics: processing journal: %w", err)
	}
	return entries, nil
}
