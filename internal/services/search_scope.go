...
//
// Поднят с 0.3 до 0.35 по замеру на живых данных (#2414): на 0.3 запрос «Траттория»
// вместе с нужной записью тянул «Трактор», и владелец справедливо назвал это бредом.
// Замеры strict_word_similarity разделяются чисто:
//
//	шум:    траттория/Трактор 0.286, тратория/Трактор 0.308, кузнецов/Кузьмин 0.214
//	нужное: ивонов/Иванов 0.400, рогалев/Роголев 0.455, смрнов/Смирнов 0.500,
//	        sergev/Sergeev 0.667, tratoria/La Trattoria 0.727
//
// 0.35 стоит посередине разрыва: весь шум ниже, все настоящие опечатки выше. Ниже
// начинают проходить общие триграммы («арбуз» к «Карбышев»), выше — перестают
// ловиться опечатки в короткой фамилии (ближайшая, «ивонов», даёт ровно 0.400).
const searchTrigramThreshold = "0.35"
