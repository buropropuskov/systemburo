...
func SetupTestApp(t *testing.T) (*echo.Echo, *gorm.DB, func()) {
	...
	testAuditRecorder := services.NewAuditRecorder(db)
	trashService := services.NewTrashService(db, testAuditRecorder)
	...
	systemTableHandler := handlers.NewSystemTableHandler(systemTableService, testAuditRecorder, 10*1024*1024, uploadDir)
