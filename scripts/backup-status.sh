...
# Глубина хранения копий и то, что из неё следует (#2357).
#
# Удаление данных из работающей системы не удаляет их из уже снятых копий, поэтому
# вопрос «на сколько назад система помнит удалённое» - это вопрос о глубине хранения
# копий, а не о сроках хранения данных. Ответ на него нужен и оператору (сколько ещё
# восстановимо), и проверяющему (сколько живут сведения после уничтожения), а взять
# его было негде: числа лежали в параметрах, и складывать их приходилось в уме.
KEEP_DAILY="${BACKUP_KEEP_DAILY:-7}"
KEEP_WEEKLY="${BACKUP_KEEP_WEEKLY:-4}"
KEEP_MONTHLY="${BACKUP_KEEP_MONTHLY:-6}"
# Горизонт - самая старая копия, которую держит расписание. Считается по самому
# длинному из трёх сроков: месячная копия за первое число переживает и суточные, и
# недельные. Месяц берём за 30 суток - точности здесь не нужно, нужен порядок.
HORIZON_DAYS=$(( KEEP_MONTHLY * 30 ))
if [ $(( KEEP_WEEKLY * 7 )) -gt "$HORIZON_DAYS" ]; then HORIZON_DAYS=$(( KEEP_WEEKLY * 7 )); fi
if [ "$KEEP_DAILY" -gt "$HORIZON_DAYS" ]; then HORIZON_DAYS="$KEEP_DAILY"; fi

echo "Глубина хранения копий: суточных ${KEEP_DAILY}, недельных ${KEEP_WEEKLY}, месячных ${KEEP_MONTHLY}"
echo "Система помнит удалённое: до ${HORIZON_DAYS} суток назад (около $(( HORIZON_DAYS / 30 )) мес)."
echo "Уничтоженные данные остаются в копиях весь этот срок. После восстановления их"
echo "снимает заново scripts/restore.sh - шаг «Применяю уничтожения заново по журналу»."
echo

# Сроки хранения самих данных - рядом, чтобы одно сверялось с другим. Оба по
# умолчанию выключены (0): установка, которая о сроке не просила, ничего не затирает.
APP_RETENTION="${APPLICATION_RETENTION_MONTHS:-0}"
AUDIT_RETENTION="${AUDIT_RETENTION_MONTHS:-0}"
echo "Сроки хранения данных в системе"
if [ "$APP_RETENTION" -gt 0 ]; then
  echo "  заявки обезличиваются через ${APP_RETENTION} мес (APPLICATION_RETENTION_MONTHS)"
else
  echo "  заявки не обезличиваются: APPLICATION_RETENTION_MONTHS не задан"
fi
if [ "$AUDIT_RETENTION" -gt 0 ]; then
  echo "  история чистится через ${AUDIT_RETENTION} мес (AUDIT_RETENTION_MONTHS)"
else
  echo "  история не чистится: AUDIT_RETENTION_MONTHS не задан"
fi
# Копии, живущие дольше самих данных, - не поломка, а то, ради чего заведён журнал
# уничтожения: закон допускает такое при условии, что после восстановления удаления
# применяются заново. Молчать об этом нельзя, иначе условие некому исполнять.
if [ "$APP_RETENTION" -gt 0 ] && [ "$HORIZON_DAYS" -ge $(( APP_RETENTION * 30 )) ]; then
  echo "  ВНИМАНИЕ: копии хранятся дольше, чем данные живут в системе. Проверьте, что"
  echo "  восстановление идёт через scripts/restore.sh, а не голым pg_restore."
fi
echo
...
# Расписание говорит, на сколько назад копии ХРАНЯТСЯ, а эта строка - на сколько
# назад они реально ЕСТЬ. Числа расходятся у молодой установки (копий ещё нет на всю
# глубину) и у сломавшегося копирования, и оператор обязан видеть именно второе.
OLDEST_DB="$(find "$BACKUP_DIR"/{daily,weekly,monthly} -maxdepth 1 -name 'buro-db-*' -type f -printf '%f\n' 2>/dev/null | sort -u | head -1)"
if [ -n "$OLDEST_DB" ]; then
  oldest_stamp="${OLDEST_DB#buro-db-}"; oldest_stamp="${oldest_stamp%%.dump*}"
  split_stamp "$oldest_stamp"
  oldest_epoch="$(date -d "${STAMP:0:4}-${STAMP:5:2}-${STAMP:8:2} ${STAMP:11:2}:${STAMP:13:2}" +%s 2>/dev/null || echo 0)"
  if [ "$oldest_epoch" -gt 0 ]; then
    echo "Самая старая копия: $(human_when "$STAMP") ($(( ( $(date +%s) - oldest_epoch ) / 86400 )) суток назад)"
    echo "Дальше этой даты восстановить нечего - и удалённого до неё в копиях уже нет."
    echo
  fi
fi
