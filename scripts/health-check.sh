...
#
# Почему вход проверяется настоящим входом, а не ответом 401 на заведомо неверный
# пароль: в internal/services/auth_service.go ЛЮБАЯ ошибка выборки пользователя -
# включая недоступную базу - сворачивается в тот же 401, что и несуществующий
# логин. Проба "ждём 401" осталась бы зелёной ровно во время аварии. Успешный
# вход подделать нечем: он требует прочитать строку пользователя, сверить пароль
# и выдать пару токенов.
#
