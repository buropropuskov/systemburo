...
Шифрование включено по умолчанию, и с ним система не стартует, пока ключи не
заданы: в бланках заявок ФИО, паспортные данные и номера патентов, и ложиться на
диск открытым текстом они не должны.
...
if [[ "$ENV" == "staging" ]]; then
    ...
    # Шифрование обязательно и на стенде: данные там демонстрационные, но режим
    # работы должен совпадать с боевым, иначе стенд перестаёт проверять установку.
    # Ключи скрипт генерирует сам несколькими строками выше.
...
# Какие узлы служб доставки разрешены. Пусто - любой наружный: адрес службы выбирает
# браузер пользователя, и список по умолчанию отрезал бы менее известные сборки.
# Адреса внутрь сети (петля, частные сети, служба метаданных) запрещены всегда,
# независимо от этой строки. Слово known раскрывается в список известных служб
# (Google, Mozilla, Microsoft, Apple), свои узлы дописываются через запятую:
# PUSH_ALLOWED_HOSTS=known,push.corp.example
PUSH_ALLOWED_HOSTS=
EOF
...
# Срок хранения персональных данных в заявках, месяцев. По истечении участники
# заявки обезличиваются: сама заявка, её даты, статус и факт прохода остаются, ФИО и
# документы затираются необратимо. 0 - выключено: срок назначает владелец системы,
# и он зависит от того, какие запросы к нему приходят (налоговые документы - 60).
APPLICATION_RETENTION_MONTHS=0
...
MAIL_MESSAGE_RETENTION_DAYS=30
EOF
