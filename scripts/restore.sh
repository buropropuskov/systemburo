...
#
# Сбой не проглатывается: при ненулевом коде восстановление останавливается общим
# обработчиком (set -e + trap on_failure), и система остаётся выключенной. Это
# сознательно - поднять её с возвращёнными персональными данными хуже, чем оставить
# в режиме технических работ и разобраться.
# Команда сама печатает, сколько применено заново, сколько не возвращалось и сколько
# требует рук; вывод идёт и на экран, и в файл - по нему ниже проверяется, есть ли
# записи, с которыми придётся разбираться вручную.
REPLAY_OUT="${WORK_DIR}/replay.log"
"${COMPOSE[@]}" run --rm -T backend ./server destruction replay -apply 2>&1 | tee "$REPLAY_OUT"
if grep -q 'ТРЕБУЕТ РУК:' "$REPLAY_OUT"; then
  echo >&2
  echo "ВНИМАНИЕ: часть уничтожений автоматически не повторяется - строки выше." >&2
  echo "Разберите их до того, как откроете пользователям доступ: до этого момента в" >&2
  echo "системе снова живут данные, которые были уничтожены." >&2
fi
