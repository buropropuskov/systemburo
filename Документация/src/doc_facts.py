...
def anchors(m):
    ...
    return [
        ...
        (OVERVIEW, "состава серверной части",
         r"Серверная часть \| ([\d  ]+?) строки без тестов: (\d+) службы, "
         r"(\d+) обработчик\w*, (\d+) моделей",
         [m["be_lines"], m["services"], m["handlers"], m["models"]]),
        (OVERVIEW, "состава интерфейса",
         r"интерфейса \| ([\d  ]+?) строки: (\d+) экранов, "
         r"(\d+) элемента", [m["vue_lines"], m["views"], m["components"]]),
    ]
